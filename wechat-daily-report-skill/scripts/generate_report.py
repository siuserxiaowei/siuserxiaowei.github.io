...
# 默认使用桌面日报网页宽度，便于生成类似 GitHub Pages 示例的群日报。
DEFAULT_VIEWPORT_WIDTH = 1180
...
def parse_arguments():
    ...
    parser.add_argument('--viewport-width', type=int, default=DEFAULT_VIEWPORT_WIDTH, help='Screenshot viewport width for image output')
    parser.add_argument('--viewport-height', type=int, default=DEFAULT_VIEWPORT_HEIGHT, help='Screenshot viewport height for image output')
    parser.add_argument('--device-scale-factor', type=float, default=DEFAULT_DEVICE_SCALE_FACTOR, help='Screenshot device scale factor')
...
def html_to_image(html_path, output_path, viewport_width, viewport_height, device_scale_factor):
    ...
    with sync_playwright() as p:
        ...
        # full_page screenshots do not play nicely with sticky sidebars after scroll.
        # Capture mode keeps the exported long image aligned while preserving sticky UI in HTML.
        page.evaluate("document.body.classList.add('is-capturing')")
